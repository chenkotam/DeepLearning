...
skip_num = 100
corr_top = 99
hold_out_ratio = 1958 - max(return_lag,flow_lag) - 252 #set as window size for no hold-out
...
for i in tqdm(range(loop_num)):
    try:
        data = x_small(returns_df, flows_df, stock_ID = sID[i], retlag=return_lag, flowlag=flow_lag)
#        y_act.append(data[hold_out_ratio:,0])
#        data = x_small_MA(returns_df, flows_df, stock_ID = sID[i], retlag=return_lag, flowlag=flow_lag, ma_range=MA_list)
#        data = x_full(returns_df, flows_df, stock_target=sID[i], stock_list=corr_sID[i], retlag=return_lag, flowlag=flow_lag)
...
err_LDA = np.where(err_LDA==0, -1, err_LDA)
err_LogitLasso = np.where(err_LogitLasso==0, -1, err_LogitLasso)
err_RF = np.where(err_RF==0, -1, err_RF)
#err_LogitLasso = np.where(err_QDA==0, -1, err_QDA)
err_benchmark = np.where(err_benchmark==0, -1, err_benchmark)

comb_matrix = np.zeros((err_benchmark.shape[0],err_benchmark.shape[1]))
for i in range(err_benchmark.shape[0]):
    for j in range(err_benchmark.shape[1]):
        if err_benchmark[i,j] + err_LDA[i,j] + err_LogitLasso[i,j] >=1:
            comb_matrix[i,j] = 1
        else:
            comb_matrix[i,j] = 0
...
for i in range(err_benchmark.shape[0]):
    for j in range(err_benchmark.shape[1]):
        if err_benchmark[i,j] + err_LDA[i,j] ==2:
            comb_matrix[i,j] = 1
        elif err_benchmark[i,j] + err_LDA[i,j]== -2:
            comb_matrix[i,j] = 0
        else:
            comb_matrix[i,j] = 0
...
'''for long only'''
err_LDA = np.where(err_LDA==-1, 0, err_LDA)
err_LogitLasso = np.where(err_LogitLasso==-1, 0, err_LogitLasso)
err_RF = np.where(err_RF==-1, 0, err_RF)
#err_LogitLasso = np.where(err_QDA==-1, 0, err_QDA)
err_benchmark = np.where(err_benchmark==-1, 0, err_benchmark)
'''------------'''

ret_LDA = err_LDA*y_act
ret_LogitLasso = err_LogitLasso*y_act
ret_benchmark = err_benchmark*y_act
ret_RF= err_RF*y_act
ret_comb = comb_matrix*y_act


ret_LDA_array = np.mean(ret_LDA, axis=1)
ret_LogitLasso_array = np.mean(ret_LogitLasso, axis=1)
ret_benchmark_array = np.mean(ret_benchmark, axis=1)
ret_RF_array = np.mean(ret_RF, axis=1)
ret_comb_array = np.mean(ret_comb, axis=1)

rf = 0.0184
sr_LDA = (np.mean(ret_LDA_array) - rf)/np.std(ret_LDA_array)
sr_LogitLasso = (np.mean(ret_LogitLasso_array) - rf)/np.std(ret_LogitLasso_array)
sr_benchmark = (np.mean(ret_benchmark_array) - rf)/np.std(ret_benchmark_array)
sr_RF = (np.mean(ret_RF_array) - rf)/np.std(ret_RF_array)
sr_comb = (np.mean(ret_comb_array) - rf)/np.std(ret_comb_array)



cum_LDA = np.cumprod(1+ret_LDA_array)
cum_LogitLasso = np.cumprod(1+ret_LogitLasso_array)
cum_benchmark = np.cumprod(1+ret_benchmark_array)
cum_RF = np.cumprod(1+ret_RF_array)
cum_comb= np.cumprod(1+ret_comb_array)

time = pd.to_datetime(returns_df['Dates'].iloc[-len(cum_benchmark):], format='%Y%m%d')
plt.figure(figsize=(10,6))
plt.plot(time, cum_LDA, label='LDA')
plt.plot(time, cum_LogitLasso, label='LogitLasso')
plt.plot(time, cum_benchmark, label='benchmark')
plt.plot(time, cum_RF, label='RF')
plt.plot(time, cum_comb, label='combination')
plt.axvline(x=time.iloc[-50], ls='dashed', c='k', lw=1.5)
plt.axvline(x=time.iloc[-200], ls='dashed', c='k', alpha=0.5, lw=1.5)
ax = plt.gca()
...
plt.legend()
plt.tight_layout()
plt.savefig('gph/Trading_Strategy.pdf')
